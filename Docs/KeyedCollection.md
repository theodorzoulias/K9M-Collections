# The `KeyedCollection<TKey,TItem>` collection

The `K9M.KeyedCollection<TKey,TItem>` collection is an alternative to the standard
[`Dictionary<TKey,TValue>`](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.dictionary-2).
It is basically a dictionary with the keys embedded in the items.
...
which is less than a .NET `Dictionary<TKey,TValue>`, because the keys are stored only once.
It provides the tools needed for efficient in-place mutations of the items, with minimal
hashings of the keys. It is implemented as a hashtable, similar to the
.NET `Dictionary<K,V>` and `HashSet<T>`, with similar performance characteristics.
After inserting an item in the collection, its key should be immutable. Every other property
of the item is allowed to change, except the key.
